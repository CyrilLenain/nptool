...
/* namespace TMinosPhysics_NS{ */
/*   TClonesArray data_result; */
/*   TMinosResult *minosdata_result; */
/*   /1* TClonesArray fitdata; *1/ */
/*   /1* TMinosClust *minosfitdata; *1/ */
/*   int NclusterFit; */
/* } */

/* using namespace TMinosPhysics_NS; */
...
void TMinosPhysics::PreTreat() {
  ...
  if(filled>0) { // Nbr of activated pad 
    while(Xpad.size()>=10 && Iteration< 20 ) {   //  !!!!!!!!  
      ...
      cout << clusterringboolTemp.back() << endl; 
  ...
  if(trackNbr>0 && trackNbr < 5 ){  ///////////
    ...
    for(unsigned int i=0 ; i< NbOfPad; i++ ) {
      ...
      // Find the T_min & T_max limits of the signal non zero
          ...
          /* fit_function->SetParLimits(0,0,10000000);// for simu */
          ...
          if(fit2DStatus!=0 || fit_function_max<=20. || 
            /* fit_function_max>=10000000 || fit_function_Tpad<=0.15 || // for simu */  
            /* fit_function_max>=10000. || fit_function_Tpad<=0.15 || */  
        ...
        if(xin.size()>0 && ((cluster_temp!=int(clusternbr[i]) && i!=0) || i==(XpadNew.size() - 1))){
          ...
          if(xout.size()>10 && ringsum>=8){
            ...
            for(unsigned int ij=0; ij<xout.size(); ij++){
              ...
              point.RotateZ(0.);//15.6 CHANGE####
              xoutprime.push_back(point.X());youtprime.push_back(point.Y());zoutprime.push_back(point.Z());
              TOTxoutprime.push_back(point.X());    // Added line to see events used for fit of lines in tree
    ...
    if(trackNbr_FINAL>= 1){   //  !!!  Just for more or 1 tracks!
    /* if(trackNbr_FINAL== 2){ */
